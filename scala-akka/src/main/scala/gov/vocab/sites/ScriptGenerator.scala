...
class ScriptGenerator {
  ...
  def generateNeo4JScriptForSites(filename: String): Unit = {
    ...
    for (row <- lines) {
      ...
      var name = row(1)
      if (row(0).indexOf('.') > 0) { //Not true for first line.. so we're good on not checking previousline
  ...
  def generateNeo4JScriptsForMorphology(filename: String): Unit = {
    ...
    for (row <- lines) {
      if (row.length == 2) {
        ...
        val name = row(1)
        pw.write(s"CREATE ($codename:MORPH_CODE { code: '$code', histology: $hist, behavior: $behavior, name: '$name'})\n")
      } else {
        val name = row(0)
        ...
        pw.write(s"CREATE ($synCode:SYNONYM { code: '$code', histology: $hist, behavior: $behavior, name: '$name'})\n")
...
object ScriptGeneratorApp extends App {
  ...
  sg.generateNeo4JScriptForSites("src/main/resources/sites.txt")
  sg.generateNeo4JScriptsForMorphology("src/main/resources/morphology.txt")
  sg.generateNeo4JScriptsForHistSiteRelationship("src/main/resources/histology_site_rel.txt")
}
